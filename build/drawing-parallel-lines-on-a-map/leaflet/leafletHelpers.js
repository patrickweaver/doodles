...
function drawMap(mapId) {
  map = L.map(mapId).setView(MAP_CENTER, MAP_ZOOM);
...
function drawTracks(stationA, stationB, stationC) {
  ...
  stationA.nOffset = offsetFromPoint(
    aPos[0],
    aPos[1],
    offsetAVector.x,
    offsetAVector.y,
  ...
  stationA.sOffset = offsetFromPoint(
    aPos[0],
    aPos[1],
    offsetAVector.x,
    offsetAVector.y,
  ...
  // Swap N and S for C:
  stationC.sOffset = offsetFromPoint(
    cPos[0],
    cPos[1],
    offsetCVector.x,
    offsetCVector.y,
  ...
  stationC.nOffset = offsetFromPoint(
    cPos[0],
    cPos[1],
    offsetCVector.x,
    offsetCVector.y,
  ...
  L.polyline([stationA.nOffset, stationA.sOffset], { color: "#00fff2" }).addTo(
    map,
  );
  L.polyline([stationC.nOffset, stationC.sOffset], { color: "#00fff2" }).addTo(
    map,
  );

  //Draw lines between offsets:
  L.polyline([stationA.nOffset, stationB.nOffset], { color: "orange" }).addTo(
    map,
  );
  L.polyline([stationB.nOffset, stationC.nOffset], { color: "orange" }).addTo(
    map,
  );
  L.polyline([stationA.sOffset, stationB.sOffset], { color: "yellow" }).addTo(
    map,
  );
  L.polyline([stationB.sOffset, stationC.sOffset], { color: "yellow" }).addTo(
    map,
  );
...
function drawVectorTracks(stationA, stationB, color) {
  ...
  //Draw lines between offsets:
  L.polyline([stationA.offsets[0], stationB.offsets[0]], {
    color: color,
    weight: 2,
  }).addTo(map);
  L.polyline([stationA.offsets[1], stationB.offsets[1]], {
    color: color,
    weight: 2,
  }).addTo(map);
}
...
function findOffsetPoints(stationA, stationB, stationC) {
  ...
  if (!stationC) {
    const offsetAVector = abVector
      ...
      .normalize()
      .rotate(Math.PI / 2);
    stationB.nOffset = offsetFromPoint(
      pos.b[0],
      pos.b[1],
      offsetAVector.x,
      offsetAVector.y,
    ...
    stationB.sOffset = offsetFromPoint(
      pos.b[0],
      pos.b[1],
      offsetAVector.x,
      offsetAVector.y,
